...
哪些模型属于 Go 档由**两层**共同决定，单靠任何一层都不够：

| 层 | 判据 | 作用 | 弱点 |
| --- | --- | --- | --- |
| 快速基线 | `isGoModel()` 的静态规则：开源供应商收录、premium 品牌排除、再加一张很短的例外表 | 不需要额外网络，目录一到就能发布 | 会过期——它曾把例外写死在 `muse-spark-1.2-contributor`，官方升到 1.3 后就静默漏掉了（#7） |
| 权威覆盖 | 官方表 `models.md` 的 **`Min plan` 列**（插件本来就为 reasoning efforts 在拉这张表） | 表到货后作为**覆盖**生效，**双向**：能补回被官方升档的，也能移除被降档的 | 表里没有的 id 不参与判定 |

设计要点：

- **目录只拉一次且不过滤**，保留完整列表。否则被基线预先排除的模型永远无法被覆盖层补回来。
- 覆盖层在 effort 元数据同一次抓取里得到（同一张表），所以**不增加网络请求**。
- 仍然**先发布基线结果**、再叠加覆盖：快速路径不受影响，冷启动不会因为等表而空目录
  （见 #4 的注册竞态）。
- 表里没有的 id 回退到基线；`Min plan` 为 `GOAT and above` / `Pro and above` / `Max`
  都不算 Go（`GOAT` 靠 `\b` 词边界排除，不会被 `go` 前缀误匹配）。

实测对齐（0.9.1，官方表 45 个 Go 档模型）：插件选择与官方表**集合完全一致，0 漏 0 误**。

## 排错

- **某个官方标记为 Go 的模型不在列表里**（#7 这类）：先看它是否已升到官方表的
  `Min plan` 列。基线例外表是硬编码的，覆盖层才是自动跟随的那一层——若官方表已标 Go
  而仍不显示，说明该模型 id 不在 `/provider/v1/models` 列表里（表里有、接口没有），
  此时插件无法收录。
