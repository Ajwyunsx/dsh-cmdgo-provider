...
export function cacheHitRate(sample: Pick<RequestStatsSample, 'inputTokens' | 'cacheReadTokens' | 'cacheWriteTokens'>): number {
...
export class RequestStats {
  ...
  record(sample: RequestStatsSample): void {
    ...
    const row: RequestStatsRow = existing ?? { ...emptyCounters(), label: key === '' ? '-' : labelFor(key), updatedAt: sample.at }
    add(row, sample)
